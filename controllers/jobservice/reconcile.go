...
import (
	...
	jobserviceresources "github.com/ovh/harbor-operator/controllers/jobservice/resources"
...
func (r *Reconciler) UpdateAppliedStatus(ctx context.Context, result *ctrl.Result, jobservice *containerregistryv1alpha2.JobService) error {
	if jobservice.Status.ObservedGeneration != jobservice.ObjectMeta.Generation {
		...
		err := r.Status.UpdateCondition(ctx, &jobservice.Status, containerregistryv1alpha2.AppliedConditionType, corev1.ConditionFalse, "new", "new generation detected")
		if err != nil {
			result.Requeue = true
	...
	switch r.Status.GetConditionStatus(ctx, &jobservice.Status, containerregistryv1alpha2.AppliedConditionType) {
	case corev1.ConditionTrue: // Already applied
		...
		if err != nil {
			...
			err := r.Status.UpdateCondition(ctx, &jobservice.Status, containerregistryv1alpha2.AppliedConditionType, corev1.ConditionFalse, err.Error())
			if err != nil {
				result.Requeue = true
	...
	default: // Not yet applied
		err := r.Status.UpdateCondition(ctx, &jobservice.Status, containerregistryv1alpha2.AppliedConditionType, corev1.ConditionFalse)
		if err != nil {
			result.Requeue = true
		...
		err = r.Status.UpdateCondition(ctx, &jobservice.Status, containerregistryv1alpha2.AppliedConditionType, corev1.ConditionTrue)
		if err != nil {
			result.Requeue = true
...
func (r *Reconciler) UpdateReadyStatus(ctx context.Context, result *ctrl.Result, jobservice *containerregistryv1alpha2.JobService) error {
	err := r.Status.UpdateCondition(ctx, &jobservice.Status, containerregistryv1alpha2.ReadyConditionType, corev1.ConditionFalse, "not-implemented", "Readiness check is not yet implemented")
	if err != nil {
		result.Requeue = true
