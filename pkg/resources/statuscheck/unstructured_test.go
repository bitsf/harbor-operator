...
var _ = Describe("Check the status", func() {
	...
	Context("Of a portal resource", func() {
		var resource *unstructured.Unstructured
		var data *containerregistryv1alpha2.Portal

		BeforeEach(func() {
			s, err := scheme.New(context.TODO())
			Expect(err).ToNot(HaveOccurred())

			data = &containerregistryv1alpha2.Portal{}
			gvks, _, err := s.ObjectKinds(data)
			Expect(err).ToNot(HaveOccurred())
		...
		Context("With empty status", func() {
			BeforeEach(func() {
				data.Status = containerregistryv1alpha2.ComponentStatus{}
			})
		...
		Context("With basic inprogress status", func() {
			Context("To True", func() {
				BeforeEach(func() {
					data.Status.Conditions = []containerregistryv1alpha2.Condition{
						{
