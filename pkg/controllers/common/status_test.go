	...
	Describe("An Harbor resource", func() {
		...
		Context("With Applied condition to True", func() {
			...
			JustBeforeEach(func() {
				...
				h.Status.Conditions = []containerregistryv1alpha2.Condition{condition}
			})
		...
		Context("With multiple conditions", func() {
			var readyCondition status.Condition
			...
			JustBeforeEach(func() {
				...
				appliedCondition := status.Condition{
					Type:   status.ConditionFailed,
					Reason: "",
					Status: corev1.ConditionTrue,
				}
				h.Status.Conditions = []containerregistryv1alpha2.Condition{readyCondition, appliedCondition}
			})
